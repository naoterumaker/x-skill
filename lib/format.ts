...
export function formatTweetTelegram(t: Tweet, index?: number): string {
  ...
  // Media indicator
  const mediaIcons = t.media.map((m) => {
    if (m.type === "photo") return "img";
    if (m.type === "video") return "vid";
    if (m.type === "animated_gif") return "gif";
    return m.type;
  });
  const mediaTag = mediaIcons.length > 0 ? ` [${mediaIcons.join(",")}]` : "";

  let out = `${prefix}@${t.username} (${engagement}${mediaTag} · ${time})\n${cleanText}`;

  if (t.url_meta.length > 0 && t.url_meta[0].title) {
    out += `\n📎 ${t.url_meta[0].title}`;
  }
  if (t.urls.length > 0) {
    out += `\n🔗 ${t.urls[0]}`;
...
export function formatTweetMarkdown(t: Tweet): string {
  ...
  // Media tags
  const mediaTags = t.media.map((m) => `[${m.type}]`).join(" ");
  const mediaStr = mediaTags ? ` ${mediaTags}` : "";

  let out = `- **@${t.username}** (${engagement}${mediaStr}) [Tweet](${t.tweet_url})\n  > ${quoted}`;

  // URL metadata with titles
  if (t.url_meta.length > 0) {
    const linkParts = t.url_meta.map((um) => {
      const label = um.title || (() => { try { return new URL(um.expanded_url).hostname; } catch { return um.expanded_url; } })();
      return `[${label}](${um.expanded_url})`;
    });
    out += `\n  Links: ${linkParts.join(", ")}`;
  } else if (t.urls.length > 0) {
    out += `\n  Links: ${t.urls.map((u) => `[${(() => { try { return new URL(u).hostname; } catch { return u; } })()}](${u})`).join(", ")}`;
  }

  // Media URLs
  if (t.media.length > 0) {
    const mediaLinks = t.media
      .filter((m) => m.url || m.preview_url)
      .map((m) => `[${m.type}](${m.url || m.preview_url})`);
